...
router.get("/memberByEmail/:email", ensureAuth, async (req, res) => {
  try {
    let member = await Member.findOne({ Email: req.params.email }) //use this function call to get dependents
      .populate("user")
      .lean(); //try removing this and look at data
    if (!member) {
      return res.status(404).send("Not found.");
